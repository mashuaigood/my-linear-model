...
fault_num = 2;
%% 用设计点参数进行归一化
for q = 1:10     %这是传感器的数量，每个都做一个故障模型
    
    
    for j =1:11
        data = eval(['load(''E:\Program Files (x86)\MATLAB\MATLAB Production Server\R2014a\toolbox\CivilEnginelibV1\linemodel\sen_data',num2str(j),'.mat'')']);
        ...
        y_stable_{j} = mean_stable(y_index);
        y_stable{j} = y_stable_{j};
        y_stable{j}(q) =  y_stable_{j}(q)+fault_s;
    ...
    eval(['save(''data_cal\\sen',num2str(q),'_P_.mat'',''P_'')']);
    eval(['save(''data_cal\\sen',num2str(q),'_Nl.mat'',''Nl'')']);
    eval(['save(''data_cal\\sen',num2str(q),'_y_stable.mat'',''y_stable'')']);
    eval(['save(''data_cal\\sen',num2str(q),'_u_stable.mat'',''u_stable'')']);
    ...
    %% save parameters
    eval(['save(''data_cal\\sen',num2str(q),'_A.mat'',''A'')']);
    eval(['save(''data_cal\\sen',num2str(q),'_B.mat'',''B'')']);
    eval(['save(''data_cal\\sen',num2str(q),'_C.mat'',''C'')']);
    eval(['save(''data_cal\\sen',num2str(q),'_D.mat'',''D'')']);
