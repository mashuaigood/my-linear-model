...
P_i = [16,22,38];%序号代表P21,P24,P3,P5
T_i = [15,27];%序号代表T21,T24,T3,T5 
for j = 1:11
...
%         PWf = Wf_norm_step;
    ...
    P_stable{j} = P_norm_stable;T_stable{j} = T_norm_stable;N_stable{j} = N_norm_stable;Wf_stable{j} = [Wf_norm_stable,data_stable(:,50:51),data_step(:,index_h+49)-1];
    ...
    P_(:,:,j) = P;
    Nl(j) = N_norm_stable(5,1);  %取一个风扇转速稳态值 
end
